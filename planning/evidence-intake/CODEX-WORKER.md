...
Read the [staging consolidation boundary](README.md#staging-consolidation-and-executable-boundary)
and [vault handoff](../consolidation/vault-handoff.md) first. The public checker is
read-only; it does not perform capture, configure private custody, or authenticate
permission. Imported schema fields and reviewer/status assertions are not evidence
that a person approved an action. Keep any private destination configuration and
locators outside public Git. This protocol-only consolidation admits no item.
...
14. **Validate.** Validate certificate schema, manifest schema, public hashes, index row, links and Git diff. Verify private commitments only on the authorized private surface with the original bytes and salts; the public checker cannot do that. Check transcription fidelity and disclosure scope separately from schemas and hashes.
...
- For a withheld private transcript, both `path` and `sha256_or_commitment` are null in the current public profile; do not put an untyped fingerprint there.
- Active exports require a recorded `public_disclosure_reviewed: true` assertion, but the flag is not authenticated approval or a permission grant.

If a safe redacted derivative exists:
- include derivative public path and SHA-256;
- include the compatible public derivative manifest described in the protocol;
